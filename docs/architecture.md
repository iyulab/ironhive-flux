...
IronHive.Flux는 **IronHive**(AI/LLM 프레임워크)와 **Flux 생태계**(FileFlux, WebFlux, FluxIndex)를 연결하는 브릿지 SDK입니다. Adapter 패턴을 기반으로 IronHive의 AI 인터페이스를 Flux 생태계의 파일 처리, 웹 크롤링, 벡터 인덱싱 서비스에 적용하여, RAG Chatbot 시나리오를 지원합니다.
...
│   └── design/
│       └── deep-research-architecture.md      # DeepResearch detailed design (historical reference)
...
```
Rag -----------> Core --> FileFlux, WebFlux, FluxIndex
```
...
### 9.1 Common Settings (Directory.Build.props)
...
### 9.2 Central Package Management (Directory.Packages.props)
...
| `0fb5303` | Move DeepResearch to ironhive-agent and upgrade dependencies |
| `60978ca` | Add CI and NuGet publish GitHub workflows |
